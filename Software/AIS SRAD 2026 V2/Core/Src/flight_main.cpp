...
vector<float> runKalmanFilter(){
	...
	for (size_t i = 0; i < measured.size(); i++) {
		...
		p = p + q;
		k = safeDivide(p, p + r);
		x = x + k * safeSubtractFloat(z, x);
        p = safeSubtractFloat(1.0f, k) * p;
...
float readAccelerometer() {
    ...
    if (ADXL314_ReadXYZ(&accel)) {
        return accel.magnitude_g;
    }
